  ...
  function initCaseApp() {
    ...
    function uploadOcrSourceImage(file, recognizedText) {
      ...
      return buildOcrUploadFile(file).then(function (uploadFile) {
        ...
        formData.append('caption', preview ? ('OCR-kilde - ' + preview) : 'OCR-kilde');
        formData.append('finding_key', '');
        formData.append('law_text', '');
        formData.append('violation_reason', '');
        formData.append('seizure_ref', '');
        formData.append('file', uploadFile, uploadFile.name || ('ocr-kilde-' + Date.now() + '.jpg'));
        return fetch('/api/cases/' + root.dataset.caseId + '/evidence', secureFetchOptions({ method: 'POST', body: formData }))
          ...
          .then(function (result) {
            if (!result.ok || !result.payload || !result.payload.ok || !result.payload.evidence) throw new Error((result.payload && result.payload.message) || 'Kunne ikke lagre OCR-bildet.');
            lastOcrEvidenceSignature = signature;
    ...
    function runOcrFromFile(file) {
      ...
      registryResult.innerHTML = 'Forbereder OCR-bilde og starter tekstlesing ...';
      return runServerOcr(file)
        ...
        .then(function (result) {
          ...
          return uploadOcrSourceImage(file, result && result.text ? result.text : '').then(function (evidence) {
            if (evidence && registryResult) {
              registryResult.innerHTML += '<div class="small muted margin-top-s">OCR-bildet er også lagret i illustrasjonsrapporten.</div>';
            }
            return result;
          });
        })
