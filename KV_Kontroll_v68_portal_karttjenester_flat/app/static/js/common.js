(function () {
  ...
  var LAYER_PANEL_PREFS_VERSION = 'v68';
  ...
  function rasterServiceList(ms, defaultUrl, renderLayers, visibleLayerLookup) {
      var services = [];
      if (ms && Array.isArray(ms.rasterServices) && ms.rasterServices.length) {
      ...
      return services.map(function (svc) {
        ...
        return {
          ...
          key: String((svc && svc.url) || defaultUrl || '').replace(/\/+$/g, '').replace(/\/export$/g, '') + '|' + ids.join(',')
        };
