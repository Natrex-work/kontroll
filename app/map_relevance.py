...
def layer_panel_group(name: Any, description: Any = '', status: Any = '') -> dict[str, Any]:
    ...
    if any(token in blob for token in ('verneomrade', 'verneområde', 'nasjonalpark', 'bunnhabitat', 'saarbar', 'sårbar')):
    ...
    if any(token in blob for token in ('gytefelt', 'gyteomrade', 'gyteområde', 'oppvekst', 'beiteomrade', 'beiteområde', 'fiskeplass', 'rekefelt', 'lassettingsplass', 'låssettingsplass', 'skjellforekomst')) or normalize_text(status) == 'fiskeriomrade' or normalize_text(status) == 'fiskeriområde':
...
def decorate_catalog_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    enriched.update(layer_panel_group(row.get('name') or '', row.get('description') or '', row.get('status') or ''))
    return enriched
