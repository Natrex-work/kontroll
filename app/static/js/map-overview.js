(function () {
  ...
  function initMapOverview() {
    ...
    function renderVisibleZones(payload) {
      ...
      var synthetic = !!(payload && payload.synthetic);
      if (zoneCount) zoneCount.textContent = items.length ? (items.length + (synthetic ? ' kartlag aktive i visningen' : ' soner synlige i kartutsnittet')) : (synthetic ? 'Ingen kartlag aktive i visningen' : 'Ingen soner synlige i kartutsnittet');
      if (!items.length) {
        zoneList.innerHTML = synthetic ? '<div class="callout">Kartet viser ingen valgte fiskerilag akkurat nå. Slå på flere kartlag eller zoom kartet.</div>' : '<div class="callout">Panorer eller zoom kartet for å vise synlige fredningssoner, stengte felt og andre reguleringsområder.</div>';
        return;
      }
      zoneList.innerHTML = capped.map(zoneRow).join('') + (items.length > capped.length ? '<div class="callout">Viser de første ' + capped.length + (synthetic ? ' kartlagene i visningen.' : ' sonene i kartutsnittet. Zoom inn for å snevre inn treffene.') + '</div>' : '');
    }
