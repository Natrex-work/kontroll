(function () {
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', function () {
      navigator.serviceWorker.register('/static/sw.js?v=1.8.30').catch(function () {});
    });
  }
  ...
  var LAYER_PANEL_PREFS_VERSION = '1.8.30';
  ...
  /**
   * Classify a layer into one of the user-facing categories used by the
   * filter chips. Returns one of:
   *   'forbud'    — Forbud / Fredning / Stengte områder
   *   'fiskeri'   — Fiskerireguleringer (regulert, kvote, åpningstider, sesong)
   *   'redskap'   — Redskap- og bruks-spesifikke reguleringer
   *   'art'       — Artsspesifikke områder (hummer, makrellstørje, kongekrabbe, …)
   *   'verne'     — Marine verneområder, korall, bunnhabitat
   *   'andre'     — Andre temalag (gytefelt, oppvekst, dybde, etc.)
   *
   * The classification is heuristic and may fall through to 'andre' if
   * no category matches.
   */
  function classifyLayerCategory(layer) {
    if (!layer) return 'andre';
    var blob = normalizeLayerSearchText([layer.name, layer.description, layer.status, layer.panel_group, layer.selection_summary].join(' '));

    // Reference / non-regulation areas → 'andre' (gytefelt, oppvekst, etc.)
    // These are useful reference info but not regulated zones per se.
    if (/gytefelt|gyteomrade|oppvekstomrade|beiteomrade|fiskeplass|skjellforekomst|havbeitelokalitet|statistikkomrade|dybde\s|sjokart/.test(blob) &&
        !/forbud|fredning|stengt|verneomrade|nullfiske|maksimalmal|tralforbud/.test(blob)) {
      return 'andre';
    }

    // Verne (highest priority among regulatory)
    if (/verneomrade|nasjonal\s*park|naturreservat|landskapsvern|marint\s*vern|korall|bunnhabitat|svamp/.test(blob)) {
      return 'verne';
    }
    // Forbud / Stengte / Fredning
    if (/forbud|fiskeforbud|fredning|fredningsomrade|stengt|nullfiske|tralforbud/.test(blob)) {
      return 'forbud';
    }
    // Art-spesifikke områder (only when actually regulated, not just reference)
    if (/hummer|makrellstorj|kongekrab|laks|sjoorret|sild|torsk|kveite|brisling|tobis|sei|piggvar/.test(blob) &&
        /regulering|regulert|omrade|kvote|sesong|maksimalmal/.test(blob)) {
      return 'art';
    }
    // Redskap-spesifikke
    if (/redskap|tral|garn|teine|line|snurrev|snurpe|snorefiske|krok|jukse|reketral|bunntral/.test(blob)) {
      return 'redskap';
    }
    // Generelle fiskerireguleringer
    if (/regulering|regulert|maksimalmal|kvoteomrade|sesong|j[\s-]?melding|forskrift|begrensning/.test(blob)) {
      return 'fiskeri';
    }
    return 'andre';
  }

  function categoryDisplayInfo(category) {
    var info = {
      'forbud':  { label: 'Forbud / Fredning', emoji: '🚫', order: 1 },
      'verne':   { label: 'Verneområder', emoji: '🛡️', order: 2 },
      'fiskeri': { label: 'Fiskerireguleringer', emoji: '🐟', order: 3 },
      'art':     { label: 'Artsspesifikke', emoji: '🦞', order: 4 },
      'redskap': { label: 'Redskap / Bruk', emoji: '⚓', order: 5 },
      'andre':   { label: 'Andre temalag', emoji: '📍', order: 6 },
    };
    return info[category] || info['andre'];
  }

  /**
   * Should this layer be available in the panel for selection?
   * More inclusive than isRestrictiveLawLayer — includes all
   * fishery-regulated zones, art-specific areas, and gear restrictions
   * so the user can toggle them on/off explicitly.
   */
  function isSelectableRegulatedLayer(layer) {
    if (!layer) return false;
    var category = classifyLayerCategory(layer);
    if (category === 'andre') {
      // Still allow if the layer was already classified as restrictive law
      return isRestrictiveLawLayer(layer);
    }
    return true;
  }
    ...
    (layers || []).forEach(function (layer) {
      ...
      if (!groupsByKey[groupKey]) {
        groupsByKey[groupKey] = {
          key: groupKey,
          label: group.label,
          order: info.order * 1000 + Number(group.order || 999),
          category: category,
      ...
      groupsByKey[groupKey].layers.push(layer);
    });
  ...
  /**
   * Compute counts per category for the chip filter UI.
   */
  function countLayersPerCategory(layers) {
    var counts = { forbud: 0, verne: 0, fiskeri: 0, art: 0, redskap: 0, andre: 0 };
    (layers || []).forEach(function (layer) {
      if (!layer || !isFinite(Number(layer.id))) return;
      if (!isSelectableRegulatedLayer(layer)) return;
  ...
  function ensureLayerPanelRoot(state, mountInfo) {
    ...
    if (!root) {
      ...
      root.innerHTML = [
        ...
        '<div class="kv-temalag-category-chips" role="group" aria-label="Filter etter kategori"></div>',
