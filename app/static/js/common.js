(function () {
  ...
  function layerPanelStorageKey(el, markerState) {
    return 'kv-temalag:' + String((markerState && markerState.layerPanelKey) || (el && el.id) || 'map');
  }
  ...
  function layerPanelGroup(layer) {
    ...
    if (blob.indexOf('verne') !== -1 || blob.indexOf('nasjonalpark') !== -1 || blob.indexOf('bunnhabitat') !== -1) return { label: 'Verneområder', key: 'verneomrader', order: 40 };
    if (blob.indexOf('tare') !== -1) return { label: 'Tare', key: 'tare', order: 50 };
    if (status.indexOf('fiskeriområde') !== -1 || status.indexOf('fiskeriomrade') !== -1 || blob.indexOf('gyte') !== -1 || blob.indexOf('oppvekst') !== -1 || blob.indexOf('fiskeplass') !== -1 || blob.indexOf('rekefelt') !== -1 || blob.indexOf('skjellforekomst') !== -1 || blob.indexOf('låssettings') !== -1 || blob.indexOf('lasettings') !== -1) {
  ...
  function visibleLayersFromPrefs(layers, prefs) {
    ...
    return (layers || []).filter(function (layer) { return !hidden[String(layer && layer.id)]; });
  }
  ...
  function syncLayerPanel(state, map, allLayers, visibleLayers) {
    ...
    if (typeof prefs.open !== 'boolean') prefs.open = !window.matchMedia('(max-width: 960px)').matches;
  ...
  function createPortalMap(el, layers, markerState) {
    ...
    var rasterLayerIds = (Array.isArray(ms.rasterLayerIds) && ms.rasterLayerIds.length ? ms.rasterLayerIds.map(function (value) { return Number(value); }) : uniqueLayerIds(renderLayers)).filter(function (value) { return isFinite(value) && visibleLayerLookup[String(value)]; });
