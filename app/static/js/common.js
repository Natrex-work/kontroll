(function () {
  ...
  var PORTAL_FEATURE_CACHE_MS = 600000;
  var PORTAL_FEATURE_CACHE_LIMIT = 240;
  var PORTAL_FETCH_CONCURRENCY = 2;
  var PORTAL_FETCH_TIMEOUT_MS = 7000;
  ...
  function fetchPortalBundle(viewKey, url, layerIds, bbox) {
    ...
    var localRead = localMap && typeof localMap.readBundle === 'function' ? localMap.readBundle(layerIds, bbox).catch(function () { return null; }) : Promise.resolve(null);
    var controller = typeof AbortController === 'function' ? new AbortController() : null;
    ...
    var request = fetch(url, controller ? { signal: controller.signal, credentials: 'same-origin' } : { credentials: 'same-origin' })
      ...
      .then(function (data) {
        var normalized = normalizeFeatureCollection(data);
        normalized.layers = Array.isArray(data && data.layers) ? data.layers : [];
        if (normalized.features && normalized.features.length) {
          portalBundleCache[viewKey] = { ts: Date.now(), data: normalized };
          trimPortalBundleCache();
          if (localMap && typeof localMap.cacheBundle === 'function') localMap.cacheBundle(layerIds, bbox, normalized).catch(function () {});
          return normalized;
        }
        return localRead.then(function (offline) {
          return offline && offline.features ? offline : normalized;
        });
      })
      .catch(function () {
        if (cached && cached.data) return cached.data;
        return localRead.then(function (offline) { return offline && offline.features ? offline : { type: 'FeatureCollection', features: [], layers: [] }; });
      })
      .finally(function () {
        if (timer) clearTimeout(timer);
  ...
  function createPortalMap(el, layers, markerState) {
    ...
    if (!state) {
      ...
      map.on('moveend zoomend', function () {
        ...
        if (typeof state.refreshLayers === 'function' && liveMarkerState.fetchFeatureDetails === true) {
          ...
          state._refreshTimer = setTimeout(function () { state.refreshLayers(); }, 650);
        }
      });
    ...
    var mapServerUrl = deriveMapServerUrl(el, layers, ms);
    var rasterLayerIds = uniqueLayerIds(layers);
    var rasterKey = mapServerUrl + '|' + rasterLayerIds.join(',');

    if (state.rasterOverlay && (!mapServerUrl || !rasterLayerIds.length)) {
      try { map.removeLayer(state.rasterOverlay); } catch (e) {}
      state.rasterOverlay = null;
      state.rasterKey = '';
    }
    if (mapServerUrl && rasterLayerIds.length) {
      if (!state.rasterOverlay || state.rasterKey !== rasterKey) {
        if (state.rasterOverlay) {
          try { map.removeLayer(state.rasterOverlay); } catch (e) {}
        }
        state.rasterOverlay = createArcGisExportLayer(mapServerUrl, {
          layerDefs: rasterLayerIds,
          opacity: isFinite(Number(ms.rasterOpacity)) ? Number(ms.rasterOpacity) : 0.8
        });
        if (state.rasterOverlay) state.rasterOverlay.addTo(map);
        state.rasterKey = rasterKey;
      } else if (state.rasterOverlay) {
        state.rasterOverlay.options.layerDefs = rasterLayerIds;
        if (state.rasterOverlay.setOpacity && isFinite(Number(ms.rasterOpacity))) state.rasterOverlay.setOpacity(Number(ms.rasterOpacity));
        if (state.rasterOverlay.redraw) state.rasterOverlay.redraw();
      }
    }
    ...
    var featureLayers = (layers || []).slice();
          ...
          var geo = L.geoJSON(data, {
            style: function (feature) {
              ...
              return {
                color: color,
                weight: isLine ? 4 : 3,
                opacity: isPoint ? 0.98 : 0.96,
                fillColor: color,
                fillOpacity: isPoint ? 0.85 : 0.28,
    ...
    return featurePromise.then(function () {
      ...
      if (shouldFetchFeatureDetails) {
        ...
        visibleFeatureSummaries = (layers || []).map(layerSummaryFromDef);
      }

      if (typeof ms.onFeaturesRendered === 'function') {
        try { ms.onFeaturesRendered({ layers: layers || [], features: visibleFeatureSummaries, bbox: bbox || null, synthetic: !shouldFetchFeatureDetails }); } catch (e) {}
      }
      ...
      var legendLayers = (shouldFetchFeatureDetails ? (layers || []).filter(function (layer) {
        var key = String(layer.id);
        return (state.featureSummariesByLayer[key] || []).length > 0;
      }) : (layers || []).slice()).sort(legendSort);
      ...
      if (!state.clickBound) {
        map.on('click', function (event) {
          ...
          var activeLayers = (el._kvPortalState && Array.isArray(el._kvPortalState.currentLayers)) ? el._kvPortalState.currentLayers : (layers || []);
          var layerIds = activeLayers.map(function (layer) { return Number(layer && layer.id); }).filter(function (value) { return isFinite(value); });
