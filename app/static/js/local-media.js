(function () {
  ...
  function reassignCase(oldCaseId, newCaseId) {
    oldCaseId = String(oldCaseId || '');
    newCaseId = String(newCaseId || '');
    if (!oldCaseId || !newCaseId) return Promise.resolve([]);
    return getAllByCase(oldCaseId).then(function (rows) {
      rows = Array.isArray(rows) ? rows : [];
      return Promise.all(rows.map(function (row) {
        var next = Object.assign({}, row || {}, { case_id: newCaseId });
        return put(next).then(function (saved) {
          if (String(saved && saved.id || '') !== String(row && row.id || '')) return remove(row.id).then(function () { return saved; });
          return saved;
        });
      }));
    });
