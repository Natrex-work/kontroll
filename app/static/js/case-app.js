  ...
  function initCaseApp() {
    ...
    function selectionProfileLayerIds() {
      ...
      if (controlSel === 'fritidsfiske') {
        if (fisherySel === 'hummer') return [3, 35, 75, 76, 77];
        if (fisherySel === 'torsk') return [3, 35, 75, 82, 83, 85, 94, 91];
        if (fisherySel === 'flatøsters') return [3, 35, 73, 75];
        if (fisherySel === 'leppefisk') return [3, 35, 75, 84];
        if (fisherySel === 'steinbit') return [75, 200];
        if (fisherySel === 'laks i sjø' || fisherySel === 'sjøørret') return [75, 87];
        return [3, 35, 75, 87, 91];
      }
    ...
    function layerAllowedBySelectionProfile(layer) {
      var preferred = selectionProfileLayerIds();
    ...
    function syncMapSelectionStatus() {
      ...
      mapSelectionStatus.innerHTML = 'Kartet viser ' + layerCount + ' fiskerirelaterte kartlag.' + summary + ' Områdevurdering og regeloppslag bruker kontrollposisjonen (rød nål), men skjuler ikke andre lag i kartet.';
    }
    ...
    function updateLocalMediaStatus(message, isError) {
      ...
      var pendingCount = pendingLocalEvidenceEntries().length;
      ...
      if (localMediaStatusText) {
        if (message) {
          ...
          localMediaStatusText.textContent = '1 bilde er lagret lokalt på enheten. Det vises i appen med en gang og synkes til saken i bakgrunnen.';
        } else {
          localMediaStatusText.textContent = pendingCount + ' bilder er lagret lokalt på enheten. De vises i appen med en gang og synkes til saken i bakgrunnen.';
        }
      }
    ...
    function localRecordToEvidence(record) {
      return {
        ...
        original_filename: record.original_filename || (record.file && record.file.name) || 'bilde.jpg',
    ...
    function postEvidenceToServer(file, payload) {
      ...
      formData.append('file', file, file.name || ('bilde-' + Date.now() + '.jpg'));
      return fetch('/api/cases/' + root.dataset.caseId + '/evidence', secureFetchOptions({ method: 'POST', body: formData }))
        ...
        .then(function (result) {
          if (!result.ok || !result.payload || !result.payload.ok || !result.payload.evidence) {
            throw new Error((result.payload && result.payload.message) || 'Kunne ikke lagre bilde i saken.');
          }
    ...
    function toUploadFile(fileOrBlob, fallbackName) {
      ...
      var blob = fileOrBlob instanceof Blob ? fileOrBlob : new Blob([fileOrBlob], { type: (fileOrBlob && fileOrBlob.type) || 'image/jpeg' });
      return Promise.resolve(new File([blob], fallbackName || ('bilde-' + Date.now() + '.jpg'), { type: blob.type || 'image/jpeg', lastModified: Date.now() }));
    }
    ...
    function buildLocalEvidenceRecord(file, payload, options) {
      ...
      return {
        ...
        created_at: Date.now(),
        sync_state: 'pending',
        source_kind: options.sourceKind || 'evidence',
        original_filename: file.name || ('bilde-' + Date.now() + '.jpg'),
    ...
    function queueLocalEvidenceUpload(file, payload, options) {
      options = options || {};
      if (!file) return Promise.reject(new Error('Velg et bilde først.'));
      ...
      if (!localMediaSupported()) {
        return buildOcrUploadFile(file).then(function (prepared) {
          return postEvidenceToServer(prepared, payload).then(function (serverEntry) {
            upsertEvidenceStateEntry(serverEntry, true);
            appendEvidenceCard(serverEntry);
            return serverEntry;
          });
        });
      }
      var record = buildLocalEvidenceRecord(file, payload, { sourceKind: options.sourceKind || 'evidence', signature: signature });
      return LocalMedia.put(record).then(function (stored) {
        ...
        appendEvidenceCard(entry);
        updateLocalMediaStatus(options.statusMessage || 'Bilde lagret lokalt på enheten.');
        if (signature) lastOcrEvidenceSignature = signature;
        setAutosaveStatus(options.autosaveMessage || 'Bilde lagret lokalt', 'is-saved');
        syncLocalMediaQueue({ onlyId: stored.id, silent: true });
        return entry;
      }).catch(function () {
        return buildOcrUploadFile(file).then(function (prepared) {
          return postEvidenceToServer(prepared, payload).then(function (serverEntry) {
    ...
    function syncOneLocalMediaRecord(record) {
      ...
      return LocalMedia.update(record.id, { sync_state: 'uploading', last_error: '' }).catch(function () { return null; }).then(function () {
        return toUploadFile(record.file, record.original_filename || ('bilde-' + Date.now() + '.jpg'));
        ...
        var message = err && err.message ? err.message : 'Kunne ikke synke bilde akkurat nå.';
        return LocalMedia.update(record.id, { sync_state: 'failed', last_error: message }).catch(function () { return null; }).then(function () {
          var failedEntry = localRecordToEvidence(Object.assign({}, record, { sync_state: 'failed', last_error: message }));
          upsertEvidenceStateEntry(failedEntry, false);
          removeEvidenceCard(record.id);
          appendEvidenceCard(failedEntry);
          updateLocalMediaStatus('Kunne ikke synke ett eller flere lokale bilder akkurat nå.', true);
          return null;
        });
      });
    }
    ...
    function syncLocalMediaQueue(options) {
      ...
      return LocalMedia.getPendingByCase(root.dataset.caseId).then(function (rows) {
        ...
        if (!options.silent) updateLocalMediaStatus('Synker ' + queue.length + ' lokale bilder til saken ...');
    ...
    function loadLocalEvidenceFromDevice() {
      ...
      return LocalMedia.getPendingByCase(root.dataset.caseId).then(function (rows) {
        ...
        updateLocalMediaStatus('Lokal bildelagring er ikke tilgjengelig i denne nettleseren.', true);
        return [];
      });
    }

    function ensureLocalMediaSyncedBeforeAction() {
      if (!pendingLocalEvidenceEntries().length) return Promise.resolve(true);
      return syncLocalMediaQueue({}).then(function () {
        if (!pendingLocalEvidenceEntries().length) return true;
        return window.confirm('Noen bilder er fortsatt bare lagret lokalt på enheten og blir ikke med i forhåndsvisning eller eksport før de er synket. Vil du fortsette likevel?');
      });
    }
    ...
    function buildOcrUploadFile(file) {
      var needsProcessing = /heic|heif/i.test(String(file && file.type || '')) || /\.(heic|heif)$/i.test(String(file && file.name || '')) || Number(file && file.size || 0) > (6 * 1024 * 1024);
      if (!needsProcessing) return Promise.resolve(file);
      return preprocessImageForOcr(file, 'document').then(function (blob) {
    ...
    function runOcrFromFile(file) {
      ...
      registryResult.innerHTML = 'Lagrer bildet lokalt på enheten og starter tekstlesing ...';
    ...
    function manualPositionText() {
      if (mapState.manualPosition) return 'Manuell kontrollposisjon er valgt. Den røde nålen og koordinatene i saken brukes når appen avgjør om kontrollen er i et forbudsområde, fredningsområde eller annen regulert sone. Blå GPS-markør skjules helt til automatisk posisjon slås på igjen.';
    ...
    function updateCaseMap(options) {
      ...
      mapState.mapServerUrl = root.dataset.portalMapserver || (caseMap && caseMap.dataset ? (caseMap.dataset.portalMapserver || '') : '');
    ...
    if (btnSyncLocalMedia) {
      btnSyncLocalMedia.addEventListener('click', function () {
        updateLocalMediaStatus('Starter synk av lokale bilder ...');
    ...
    if (evidenceGrid) {
      evidenceGrid.addEventListener('click', function (event) {
        ...
        if (syncButton) {
          updateLocalMediaStatus('Starter synk av valgt bilde ...');
        ...
        if (deleteButton) {
          ...
          if (!window.confirm('Fjerne dette lokale bildet fra enheten?')) return;
    ...
    window.addEventListener('online', function () { syncLocalMediaQueue({ silent: true }); });

    syncOptions();
    updateCaseMap();
    setTimeout(function () { if (currentStep === 2) maybeAutoStartLocation(); }, 250);
    renderFindings();
    loadLocalEvidenceFromDevice().then(function () { syncLocalMediaQueue({ silent: true }); });
