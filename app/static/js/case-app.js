(function () {
  ...
  function initCaseApp() {
    ...
    var mapFilterStorageKey = 'kv-map-layer-filter-v61:' + root.dataset.caseId;
    ...
    function visibleFeatureDetailLayerIds(limit) {
    ...
    function syncMapSelectionStatus() {
      ...
      mapSelectionStatus.innerHTML = 'Kartet viser ' + layerCount + ' fiskerirelaterte kartlag og områdegrenser direkte i kartet.' + summary + ' Stengte områder, fredningsområder og andre fiskerilag vises som i kartportalen. Trykk på et område i kartet for å se informasjon og datakilde. Områdevurdering og regeloppslag bruker kontrollposisjonen (rød nål).';
    }
    ...
    function currentOfflineWarmKey() {
      ...
      var layerIds = filteredMapCatalog().map(function (layer) { return Number(layer.id); }).filter(function (value) { return isFinite(value); }).sort(function (a, b) { return a - b; });
    ...
    function prefetchVisibleMapTiles() {
      if (!caseMap || !caseMap._kvLeafletMap) return Promise.resolve(0);
      var map = caseMap._kvLeafletMap;
      var urls = [];
      map.eachLayer(function (layer) {
        if (typeof layer.getTileUrl !== 'function') return;
        urls = urls.concat(collectTileUrls(layer, map, 1));
      });
      urls = uniqueUrls(urls);
      return prefetchUrlsToCache(urls, 'kv-kontroll-v62-map-tiles');
    }

    function downloadCurrentMapToDevice() {
      if (!btnDownloadMapOffline) return Promise.resolve();
      var bbox = currentMapBbox();
      var layerIds = filteredMapCatalog().map(function (layer) { return Number(layer.id); }).filter(function (value) { return isFinite(value); });
      if (!bbox || !layerIds.length) {
        if (mapOfflineStatus) mapOfflineStatus.textContent = 'Kunne ikke laste ned kartdata akkurat nå.';
        return Promise.resolve();
      }
      if (mapOfflineStatus) mapOfflineStatus.textContent = 'Lagrer offline-kartpakke på enheten ...';
      var url = '/api/map/offline-package?bbox=' + encodeURIComponent(bbox.join(',')) + '&layer_ids=' + encodeURIComponent(layerIds.join(',')) + '&expand=1.6';
      return fetch(url, { credentials: 'same-origin' })
        .then(function (response) { return response.json(); })
        .then(function (payload) {
          var bundle = payload && payload.bundle ? payload.bundle : { type: 'FeatureCollection', features: [], layers: [] };
          var cachePromise = Promise.resolve(bundle);
          if (window.KVLocalMap && typeof window.KVLocalMap.cacheOfflinePackage === 'function') {
            cachePromise = window.KVLocalMap.cacheOfflinePackage(layerIds, bbox, payload).catch(function () { return bundle; }).then(function () { return bundle; });
          }
          return cachePromise.then(function () {
            return prefetchVisibleMapTiles().then(function (tileCount) {
              return { bundle: bundle, tileCount: tileCount, offlineBBox: payload && payload.offline_bbox ? payload.offline_bbox : bbox };
            });
          });
        })
        .then(function (result) {
          var featureCount = result && result.bundle && Array.isArray(result.bundle.features) ? result.bundle.features.length : 0;
          var tileCount = result && isFinite(Number(result.tileCount)) ? Number(result.tileCount) : 0;
          if (mapOfflineStatus) mapOfflineStatus.textContent = 'Offline-kart lagret på enheten. ' + featureCount + ' kartobjekter og ' + tileCount + ' kartbilder er klare for dette området.';
        })
        .catch(function () {
          if (mapOfflineStatus) mapOfflineStatus.textContent = 'Kunne ikke laste ned offline-kart akkurat nå.';
        });
    }
    ...
    function updateCaseMap(options) {
      ...
      var displayLayers = filteredMapCatalog();
      ...
      mapState.featureDetailLayerIds = visibleFeatureDetailLayerIds(18);
    ...
    function localSummaryFromFindings(payload) {
      var avvik = (payload && payload.findings ? payload.findings : []).filter(function (item) {
      ...
      if (payload && payload.area_name && payload.area_status && String(payload.area_status).toLowerCase() !== 'ingen treff') {
        introParts.push('Valgt kontrollposisjon ligger i eller ved ' + String(payload.area_name).trim() + ' (' + String(payload.area_status).trim() + ').');
      }
      var points = avvik.map(function (item, idx) {
        var note = String(item.notes || item.auto_note || item.summary_text || '').trim();
        var base = String(item.label || item.key || ('Avvik ' + (idx + 1))).trim();
