(function () {
  ...
  function initCaseApp() {
    ...
    function syncMapSelectionStatus() {
      ...
      mapSelectionStatus.innerHTML = 'Kartet viser ' + layerCount + ' fiskerirelaterte kartlag.' + summary + ' Trykk på et område i kartet for å se informasjon og datakilde. Områdevurdering og regeloppslag bruker kontrollposisjonen (rød nål).';
    }
    ...
    function downloadCurrentMapToDevice() {
      ...
      if (mapOfflineStatus) mapOfflineStatus.textContent = 'Laster ned kartdata til enheten ...';
      var url = '/api/map/bundle?bbox=' + encodeURIComponent(bbox.join(',')) + '&layer_ids=' + encodeURIComponent(layerIds.join(','));
      return fetch(url, { credentials: 'same-origin' })
        .then(function (response) { return response.json(); })
        .then(function (payload) {
          if (window.KVLocalMap && typeof window.KVLocalMap.cacheBundle === 'function') {
            return window.KVLocalMap.cacheBundle(layerIds, bbox, payload).catch(function () { return payload; }).then(function () { return payload; });
          }
          return payload;
        })
        .then(function (payload) {
          var featureCount = payload && Array.isArray(payload.features) ? payload.features.length : 0;
          if (mapOfflineStatus) mapOfflineStatus.textContent = 'Kartdata lagret på enheten. ' + featureCount + ' kartobjekter er klare for denne visningen.';
        })
        .catch(function () {
          if (mapOfflineStatus) mapOfflineStatus.textContent = 'Kunne ikke laste ned kartdata til enheten akkurat nå.';
        });
    }
    ...
    function updateCaseMap(options) {
      ...
      mapState.featureDetailLayerIds = filteredMapCatalog().map(function (layer) { return Number(layer.id); }).filter(function (value) { return isFinite(value); });
      ...
      createPortalMap(caseMap, filteredMapCatalog(), mapState).then(function () {
        ...
        mapState._offlineWarmTimer = setTimeout(function () { downloadCurrentMapToDevice(); }, 900);
      });
      syncManualPositionNotice();
