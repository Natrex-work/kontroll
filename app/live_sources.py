...
PORTAL_LAYER_SCHEMA_VERSION = os.getenv('KV_PORTAL_LAYER_SCHEMA_VERSION', 'v56')
...
def portal_layer_catalog(*, fishery: str = '', control_type: str = '', gear_type: str = '') -> list[dict[str, Any]]:
    ...
    if fishery or control_type or gear_type:
        ...
        if preferred_ids:
            rows = [row for row in rows if row.get('id') is not None and int(row.get('id')) in preferred_ids]
    return rows
