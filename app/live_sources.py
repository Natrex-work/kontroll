...
PORTAL_LAYER_SCHEMA_VERSION = os.getenv('KV_PORTAL_LAYER_SCHEMA_VERSION', 'v58')
...
def portal_layer_catalog(*, fishery: str = '', control_type: str = '', gear_type: str = '') -> list[dict[str, Any]]:
    ...
    if fishery or control_type or gear_type:
        rows = sorted(rows, key=lambda row: (-_portal_selection_score(row, fishery=fishery, control_type=control_type, gear_type=gear_type)[0], _portal_selection_score(row, fishery=fishery, control_type=control_type, gear_type=gear_type)[1]))
    return rows
...
def fetch_portal_bundle(*, layer_ids: list[int] | None = None, bbox: tuple[float, float, float, float] | None = None, max_age_seconds: int = 6 * 3600) -> dict[str, Any]:
    rows = portal_layer_catalog()
    if layer_ids:
        requested = {int(value) for value in layer_ids if str(value).strip()}
        rows = [row for row in rows if row.get('id') is not None and int(row.get('id')) in requested]
