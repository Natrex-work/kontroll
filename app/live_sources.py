...
PORTAL_LAYER_SCHEMA_VERSION = os.getenv('KV_PORTAL_LAYER_SCHEMA_VERSION', 'v62')
...
def refresh_portal_layer_catalog(force: bool = False, max_age_seconds: int = 6 * 3600) -> list[dict[str, Any]]:
    ...
    try:
        ...
        portal_parent_groups = {
            'administrasjon',
            'fiskereguleringer_og_vern',
            'fiskerireguleringer',
            'vern',
            'kystnaere_fiskeridata',
            'statistikkomraader',
            'tapte_redskap',
        }
        relevant_parent_ids = {
            int(layer.get('id'))
            for layer in layers
            if _ascii_header(layer.get('name')) in portal_parent_groups and layer.get('subLayerIds')
        }
