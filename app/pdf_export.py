...
# ---- 1.8.29: politifaglige standardtekster, objektiv/kildebevisst rapportstil ----
...
_AUTOGEN_BASIS_MARKERS_1_8_29 = _AUTOGEN_BASIS_MARKERS_1_8_27 + (
...
def _compact_ws_1_8_29(value: Any) -> str:
    text = re.sub(r'\s+', ' ', str(value or '')).strip()
    text = re.sub(r'\s+([,.;:])', r'\1', text)
    text = re.sub(r'\s*/\s*', '/', text)
    return text
...
def _sanitize_formal_text_1_8_29(value: Any) -> str:
    text = _compact_ws_1_8_29(value)
    replacements = [
...
def _looks_autogenerated_basis_1_8_29(text: Any) -> bool:
    low = str(text or '').strip().lower()
    if not low:
        return True
    if _looks_autogenerated_basis_1_8_27(low):
        return True
    return any(marker in low for marker in _AUTOGEN_BASIS_MARKERS_1_8_29)


def _case_when_1_8_29(case_row: Dict[str, Any]) -> str:
    when = _case_time_phrase_1_8_23(case_row)
    return when if when and when != '-' else 'kontrolltidspunktet'


def _case_place_1_8_29(case_row: Dict[str, Any]) -> str:
    place = _case_place_1_8_23(case_row)
    return place if place and place != '-' else 'kontrollstedet'


def _topic_narrative_1_8_29(case_row: Dict[str, Any]) -> str:
    control = _topic_part_1_8_26(case_row.get('control_type'))
    species = _topic_part_1_8_26(case_row.get('species') or case_row.get('fishery_type'))
    gear = _topic_part_1_8_26(case_row.get('gear_type'))
    parts: list[str] = []
    ...
    if gear:
        parts.append('med ' + gear.lower())
    topic = _compact_ws_1_8_29(' '.join(parts)).strip(' .')
    return topic or 'aktuelle fiskeri- og redskapsbestemmelser'


def _subject_name_1_8_29(case_row: Dict[str, Any], *, fallback: str = 'ukjent gjerningsperson') -> str:
    return _first_clean_1_7(
        case_row.get('suspect_name'),
        case_row.get('responsible_name'),
        case_row.get('vessel_name'),
...
def _case_vessel_unit_1_8_29(case_row: Dict[str, Any]) -> str:
    unit = _case_vessel_unit_1_8_23(case_row)
    return unit if unit and unit != '-' else 'Kystvakten'


def _source_sentence_1_8_29(case_row: Dict[str, Any]) -> str:
    source = _first_clean_1_7(case_row.get('basis_source_name'))
    return f' fra {source}' if source else ''


def _purpose_focus_1_8_29(case_row: Dict[str, Any], findings: list[Dict[str, Any]] | None = None) -> str:
...
def _area_sentence_1_8_29(case_row: Dict[str, Any]) -> str:
    area = _area_context_1_8_27(case_row)
    if not area:
        return ''
...
def _clean_custom_basis_1_8_29(case_row: Dict[str, Any], findings: list[Dict[str, Any]], text: Any) -> str:
    cleaned = _sanitize_formal_text_1_8_29(_clean_standard_text_1_7(text, case_row))
    if not cleaned:
        return _generated_control_basis_1_8_29(case_row, findings)
    basis = _normal_case_basis_1_8_22(case_row.get('case_basis'))
    if basis == 'tips' and 'patruljens egne observasjoner' not in cleaned.lower():
        cleaned = (f'Kontrollen ble gjennomført på bakgrunn av tips/opplysninger{_source_sentence_1_8_29(case_row)}. '
...
def _generated_control_basis_1_8_29(case_row: Dict[str, Any], findings: list[Dict[str, Any]]) -> str:
    basis = _normal_case_basis_1_8_22(case_row.get('case_basis'))
    unit = _case_vessel_unit_1_8_29(case_row)
    when = _case_when_1_8_29(case_row)
    place = _case_place_1_8_29(case_row)
    topic = _topic_narrative_1_8_29(case_row)
    focus = _purpose_focus_1_8_29(case_row, findings)
    if basis == 'tips':
        text = (f'{unit} gjennomførte den {when} fiskerioppsyn ved {place} på bakgrunn av mottatte tips/opplysninger{_source_sentence_1_8_29(case_row)}. '
        ...
        text = (f'{unit} gjennomførte den {when} fiskerioppsyn ved {place}. '
                f'Formålet var å føre kontroll med {topic} og avklare om {focus} var i samsvar med gjeldende regelverk.')
    text += _area_sentence_1_8_29(case_row)
    return _clean_standard_text_1_7(_sanitize_formal_text_1_8_29(text), case_row)


def build_control_reason(case_row: Dict[str, Any], findings: list[Dict[str, Any]]) -> str:  # type: ignore[override]
    explicit = str(case_row.get('basis_details') or '').strip()
    if explicit and not _looks_autogenerated_basis_1_8_29(explicit):
        return _clean_custom_basis_1_8_29(case_row, findings, explicit)
    return _generated_control_basis_1_8_29(case_row, findings)
...
def _norm_key_1_8_29(value: Any) -> str:
    return re.sub(r'[^a-z0-9æøå]+', '', str(value or '').lower())


def _dedupe_report_lines_1_8_29(rows: list[str]) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    for row in rows:
        text = _compact_ws_1_8_29(row).strip(' ;,.-')
        if not text:
            continue
        key = _norm_key_1_8_29(text)
        if key in seen:
            continue
        seen.add(key)
        out.append(text)
...
def _finding_note_for_report_1_8_29(item: Dict[str, Any], title: str) -> str:
    note = _finding_note_for_report_1_8_27(item, title)
...
def _avvik_rows_1_8_29(findings: list[Dict[str, Any]]) -> list[tuple[str, str]]:
    rows: list[tuple[str, str]] = []
    seen: set[str] = set()
    for item in _avvik_1_7(findings):
        title = _finding_title_1_8_27(item)
        note = _finding_note_for_report_1_8_29(item, title)
        key = _norm_key_1_8_29(title + '|' + note)
        if key in seen:
            continue
        seen.add(key)
...
def _short_avvik_lines_1_8_24(findings: list[Dict[str, Any]], *, limit: int = 220) -> list[str]:  # type: ignore[override]
    rows = _avvik_rows_1_8_29(findings)
    if not rows:
        return ['Det er ikke registrert avvik i kontrollpunktene på tidspunktet rapporten ble laget.']
    out: list[str] = []
    for idx, (title, note) in enumerate(rows, start=1):
...
def _legal_head_list_1_8_29(case_row: Dict[str, Any], findings: list[Dict[str, Any]], *, max_items: int = 4) -> str:
    refs = _registered_avvik_ref_rows_1_8_21(case_row, findings)
    heads: list[str] = []
    seen: set[str] = set()
    for ref in refs:
...
def _law_excerpt_1_8_23(ref: Dict[str, Any], limit: int = 200) -> str:  # type: ignore[override]
    ...
    for sent in sentences[:5]:
        if re.search(r'\b(skal|forbudt|ikke tillatt|plikter|må|krav|straffes|straffbart|tillatt|forbud)', sent, flags=re.IGNORECASE):
            ...
            break
    excerpt = normative or (sentences[0] if sentences else text)
    excerpt = _compact_ws_1_8_29(excerpt)
    if len(excerpt) > limit:
        excerpt = excerpt[:limit].rsplit(' ', 1)[0].rstrip(' ,.;:') + '...'
    return excerpt.strip()
...
def _offence_fact_line_1_8_25(block: Dict[str, Any], idx: int) -> str:  # type: ignore[override]
    ...
    details = re.sub(r'\bkontrollposisjon[^.]*\.?', '', details, flags=re.IGNORECASE).strip()
    details = _dedupe_fact_fragments_1_8_27(details)
    if details and _norm_key_1_8_29(title) not in _norm_key_1_8_29(details):
        text = f'{title}: {details}'
    else:
        text = title
    return f'{idx}. {_shorten_sentence_1_8_21(_sentence_1_7(text), 260)}'
...
def _build_short_complaint(case_row: Dict[str, Any], findings: list[Dict[str, Any]], sources: list[Dict[str, Any]]) -> str:  # type: ignore[override]
    ...
    if override:
        return _clean_standard_text_1_7(_strip_doc_heading_1_8_23(override), case_row)
    subject = _subject_name_1_8_29(case_row)
    when = _case_when_1_8_29(case_row)
    place = _case_place_1_8_29(case_row)
    unit = _case_vessel_unit_1_8_29(case_row)
    topic = _topic_narrative_1_8_29(case_row)
    offences = _offence_blocks(case_row, findings)
    titles = _offence_titles_1_8_25(case_row, findings)
    avvik_rows = _avvik_rows_1_8_29(findings)
    ...
    lines.append('')
    lines.append(f'Forholdet kan etter sin art være relevant for vurdering etter {_legal_head_list_1_8_29(case_row, findings)}. Endelig rettslig vurdering tilligger påtalemyndigheten.')
...
def _control_execution_sentence_1_8_29(case_row: Dict[str, Any], findings: list[Dict[str, Any]]) -> str:
    topic = _topic_narrative_1_8_29(case_row)
...
def _build_own_report(case_row: Dict[str, Any], findings: list[Dict[str, Any]]) -> str:  # type: ignore[override]
    ...
    topic = _topic_narrative_1_8_29(case_row)
    crew = _crew_text(case_row)
    subject = _subject_name_1_8_29(case_row, fallback='')
    paragraphs: list[str] = []
    paragraphs.append(f'Den {when} gjennomførte {unit} fiskerioppsyn ved {place}. Jeg, {investigator}, deltok i kontrollen. Kontrolltema var {topic}.')
    if crew and crew != '-':
        paragraphs.append(f'Øvrig personell/observatører som deltok i kontrollen: {crew}.')
    paragraphs.append(_own_report_basis_1_8_25(case_row, findings))
    paragraphs.append(_control_execution_sentence_1_8_29(case_row, findings))
    if subject:
    ...
    text = '\n\n'.join(_dedupe_report_lines_1_8_29([p for p in paragraphs if p]))
    for bad in _BAD_REPORT_PHRASES_1_8_29:
...
def _build_interview_report(case_row: Dict[str, Any]) -> str:  # type: ignore[override]
    ...
    for entry in entries:
        ...
        role = _first_clean_1_7(entry.get('role'), 'mistenkt/siktet')
        method = _first_clean_1_7(entry.get('method'), 'ikke oppgitt')
        place = _first_clean_1_7(entry.get('place'), _case_place_1_8_29(case_row))
        start = _fmt_datetime(entry.get('start') or case_row.get('start_time'))
        end = _fmt_datetime(entry.get('end') or case_row.get('end_time'))
        if used > 1:
            lines.append('')
...
def build_summary(case_row: Dict[str, Any], findings: list[Dict[str, Any]]) -> str:  # type: ignore[override]
    ...
    place = _case_place_1_8_29(case_row)
    lines: list[str] = [
        f'Kontrollen ble gjennomført {when} ved {place}. Kontrolltema var {_topic_narrative_1_8_29(case_row)}.',
        _own_report_basis_1_8_25(case_row, findings),
    ]
    if _avvik_1_7(findings):
        lines.append('Registrerte avvik:')
...
def build_text_drafts(case_row: Dict[str, Any], findings: list[Dict[str, Any]]) -> Dict[str, str]:  # type: ignore[override]
    return {
        ...
        'notes': '',
        'complaint_preview': _build_short_complaint(case_row, findings, _safe_sources(case_row)),
        'source_label': 'straffesaksmal 1.8.29',
    }

# 1.8.29a: justeringer etter test - behold avsnitt/linjeskift og fjern koordinater i bildetekster.

def _purpose_focus_1_8_29(case_row: Dict[str, Any], findings: list[Dict[str, Any]] | None = None) -> str:  # type: ignore[override]
    species = str(case_row.get('species') or case_row.get('fishery_type') or '').lower()
    gear = str(case_row.get('gear_type') or '').lower()
    keys = {str((item or {}).get('key') or '').lower() for item in (findings or []) if isinstance(item, dict)}
    titles = ' '.join(_finding_title_1_8_27(item).lower() for item in (findings or []) if isinstance(item, dict))
...
def _strip_coords_1_8_29(value: Any) -> str:
    text = str(value or '')
...
def _finding_note_for_report_1_8_29(item: Dict[str, Any], title: str) -> str:  # type: ignore[override]
    note = _finding_note_for_report_1_8_27(item, title)
    note = _strip_coords_1_8_29(_strip_position_phrases_1_8_24(_strip_inline_seizure_text_1_8_24(note)))
    note = re.sub(r'^' + re.escape(str(title or '')) + r'[:.\s-]*', '', note, flags=re.IGNORECASE).strip()
    note = _dedupe_fact_fragments_1_8_27(note)
    return _shorten_sentence_1_8_21(note, 220).rstrip('.')


def _dedupe_paragraphs_1_8_29(paragraphs: list[str]) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
...
def _build_short_complaint(case_row: Dict[str, Any], findings: list[Dict[str, Any]], sources: list[Dict[str, Any]]) -> str:  # type: ignore[override]
    ...
    lines.append('')
    if count > 1:
        lines.append(f'Forholdene kan etter sin art være relevante for vurdering etter {_legal_head_list_1_8_29(case_row, findings)}. Endelig rettslig vurdering tilligger påtalemyndigheten.')
    else:
        lines.append(f'Forholdet kan etter sin art være relevant for vurdering etter {_legal_head_list_1_8_29(case_row, findings)}. Endelig rettslig vurdering tilligger påtalemyndigheten.')
    lines.append('For nærmere detaljer om faktiske observasjoner, redskap/beslag og sikret bildedokumentasjon vises det til sakens ' + _evidence_doc_phrase_1_8_26(case_row) + '.')
    return _clean_standard_text_1_7('\n'.join(lines).strip(), case_row)
...
def _build_own_report(case_row: Dict[str, Any], findings: list[Dict[str, Any]]) -> str:  # type: ignore[override]
    ...
    if _has_interview_report_content_1_8_21(case_row):
        paragraphs.append('Gjennomført avhør/forklaring er protokollert i egen avhørsrapport.')
    notes = _strip_coords_1_8_29(_strip_position_phrases_1_8_24(_strip_inline_seizure_text_1_8_24(_strip_generated_report_noise_1_8_21(case_row.get('notes')))))
    if notes:
        paragraphs.append('Kontrollørs merknad: ' + _shorten_sentence_1_8_21(notes, 340))
    text = '\n\n'.join(_dedupe_paragraphs_1_8_29(paragraphs))
    for bad in _BAD_REPORT_PHRASES_1_8_29:
        text = re.sub(r'\b' + re.escape(bad) + r'\b', '', text, flags=re.IGNORECASE)
    return _clean_standard_text_1_7(text.strip(), case_row)
...
def _image_caption_base_1_8_24(item: Dict[str, Any], idx: int) -> str:  # type: ignore[override]
    raw = _first_clean_1_7(item.get('caption'), item.get('original_filename'), item.get('filename'), f'foto {idx}')
    raw = _strip_coords_1_8_29(_strip_position_phrases_1_8_24(_strip_inline_seizure_text_1_8_24(raw)))
    raw = re.sub(r'\s+', ' ', raw).strip(' .')
    if not raw:
        raw = f'foto {idx}'
    low = raw.lower()
