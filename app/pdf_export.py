...
def _structured_case_points(case_row: Dict[str, Any], findings: list[Dict[str, Any]]) -> list[str]:
    ...
    for item in _avvik_findings(findings):
        ...
        shared_note = str(item.get('notes') or '').strip()
        attached = False
        ...
        _append_point_detail(generic_facts, _finding_display_note(item) or item.get('label') or item.get('key') or 'Registrert avvik')
